# scripts/train_qrdqn_row_deepsets_fixed_opp.py
import argparse
import csv
import json
import os
...
class RowDeepSetsQRDQNNet(nn.Module):
    ...
    def forward(self, x):
        ...
        row_feats = row_feats_flat.reshape(-1, 4, 3)

        row_emb = self.phi_row(row_feats.reshape(-1, 3))
...
def qrdqn_train_step(online_net, target_net, optimizer, batch, gamma: float, kappa: float = 1.0):
    ...
    with torch.no_grad():
        next_online_quantiles = online_net(next_obs)
        next_online_q = next_online_quantiles.mean(dim=-1)
        next_online_q = next_online_q.masked_fill(~next_masks, -1e9)
        next_actions = torch.argmax(next_online_q, dim=1)

        next_target_quantiles = target_net(next_obs)[torch.arange(obs.shape[0]), next_actions]
...
# ============================================================
# Fixed-opponent env builder
# ============================================================

def build_fixed_opponent_policies(args):
...
def make_env(args, opponent_policies=None):
    if opponent_policies is None:
        opponent_policies = build_fixed_opponent_policies(args)
...
@torch.no_grad()
def evaluate(model, adapter, device: torch.device, eval_episodes: int, eval_seed: int, args, opponent_policies):
    env = make_env(args, opponent_policies=opponent_policies)
    returns = []
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="outputs/qrdqn_row_deepsets_fixed_opp_runs")
    ...
    # fixed opponents
    parser.add_argument("--opponent_name", type=str, default="ppo1_ppo2_heur1")
    ...
    if args.run_name is None:
        ...
        args.run_name = f"qrdqn_rowdeepsets_fixedopp_{args.opponent_name}_{timestamp}"
    ...
    config["train_env"] = "SixQuiPrendEnvFixedOpponents"
    ...
    final_summary = {
        ...
        "heuristic_checkpoint": args.heuristic_checkpoint,
    }
