# scripts/eval_qrdqn_row_deepsets_fixed_opp.py
import argparse
import csv
import json
import os
...
def build_fixed_opponent_policies(config, opponent_device: str):
    ppo1 = FrozenMaskablePPOPolicy.from_run_dir(
        run_dir=config["ppo1_run_dir"],
        checkpoint=config.get("ppo_checkpoint", "best_model"),
        device=opponent_device,
        deterministic=True,
    )
    ppo2 = FrozenMaskablePPOPolicy.from_run_dir(
        run_dir=config["ppo2_run_dir"],
        checkpoint=config.get("ppo_checkpoint", "best_model"),
        device=opponent_device,
        deterministic=True,
    )
    heur = FrozenHeuristicPPOPolicy.from_run_dir(
        run_dir=config["heuristic_run_dir"],
        checkpoint=config.get("heuristic_checkpoint", "best_finetuned_model"),
        device=opponent_device,
        deterministic=True,
    )
...
def make_env(config, opponent_device: str):
    opponent_policies = build_fixed_opponent_policies(config, opponent_device)
    env = SixQuiPrendEnvFixedOpponents(
        opponent_policies=opponent_policies,
        opponent_name=config.get("opponent_name", "ppo1_ppo2_heur1"),
    )
    return env
...
def main():
    ...
    env = make_env(config, args.opponent_device)
    returns = []
    ...
    summary = {
        ...
        "opponent_name": config.get("opponent_name", "ppo1_ppo2_heur1"),
        "ppo1_run_dir": config.get("ppo1_run_dir"),
        "ppo2_run_dir": config.get("ppo2_run_dir"),
        "heuristic_run_dir": config.get("heuristic_run_dir"),
    }
